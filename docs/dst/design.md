...
**Per-boot host identity (`/proc/sys/kernel/random/boot_id`) and the uptime clocks (enforced).** A
machine's kernel also carries per-**boot** state, and the procfs surface serves its identity leaf:
`/proc/sys/kernel/random/boot_id` is a UUID constant within one boot, shared by every process on the
machine, regenerated by the kernel at every boot — the discriminator a client stamps into shared
state to detect that a reboot invalidated coordination state established under the previous boot
(cross-boot epoch invalidation). Under a run the leaf serves a UUID derived deterministically from
(run seed, host, boot count): constant across re-reads and across co-located processes, and
distinct per (host, boot, seed) by 122-bit deterministic derivation — 6 of the 128 bits carry the
UUID version/variant, so distinctness is overwhelming-probability, not structurally guaranteed,
exactly as for a real kernel's random boot_id. A machine **boots** at its first declaration in the run (explicit
`Host` or implicit-host `Process`) and at its first `Host` re-declaration after `CrashHost` powered
it off; a `Host` re-declaration of a machine that is UP re-establishes its clock but does **not**
reboot it — its processes survive that declaration, and a boot_id changing under a live process is a
state no kernel shows. The value has the RFC 4122 version-4 shape a real boot_id has (a
deterministic mix of the seed, not cryptographic randomness — the entropy seam remains
`crypto/rand`'s). The leaf is read-only (open for write is `EACCES`), stats size 0 / mode `0444`
like Linux procfs, answers `ENOTDIR` under a trailing slash, and — as a regular file — `readlink`
on it (and on every modeled proc leaf that is not the `ns/pid` symlink) is `EINVAL`. Its `SameFile`
identity is per (host, boot): each boot mounts a fresh procfs instance, and two machines never share
one. The never-declared root host (host 0, the driver's machine) boots when the run starts and is
never powered off, so its boot_id is the seed's boot-1 value for the whole run.

Boot state also anchors the **uptime clocks**: raw `clock_gettime(CLOCK_MONOTONIC)` and
`clock_gettime(CLOCK_BOOTTIME)` (the interception boundary's selected clock family) read the virtual
base clock REBASED to the calling goroutine's host's boot instant — a machine's uptime is 0 at boot,
advances with virtual time, and RESETS at reboot, so a stamp taken before a power cycle compares as
"in the future" of a reading taken after it, exactly the relation a real reboot produces (the hazard
cross-boot epoch invalidation exists to catch). Wall skew/step/drift never move these clocks —
neither is settable on Linux. `CLOCK_BOOTTIME` still coincides with `CLOCK_MONOTONIC`: the two
diverge only across suspend, which the simulation does not model, so equality is the faithful
collapse and one boot origin resets both (the settled no-suspend-model decision, unchanged by the
per-boot origin). The root host's boot instant is the run epoch, so the run body's own uptime starts
at 0. Go-internal monotonic readings (`time.Now`'s hidden monotonic, timers) stay on the un-rebased
base clock: no absolute value of that clock is program-observable — only differences are — and the
observable absolute uptime (the raw clock family; no procfs uptime surface is modeled) is
host-rebased, so the split is observationally coherent. The machine's powered-off truth that gates booting is owned by
`testing/simulation` itself (not the net layer), so boot semantics — and the refusal to restart a
`Process` on a powered-off machine — hold identically in binaries that never link `net`.
...
- **The generic trampolines** `Syscall`/`Syscall6`/`RawSyscall`/`RawSyscall6` are fenced the same
  ...
  returns the DST virtual base clock rebased to the calling goroutine's host's boot instant (the
  uptime semantics of the identity section's per-boot host state), and
...
- **INV-VANILLA**: for a probe corpus of programs built untagged — same GOOS/GOARCH,
  ...
  widened record, and the renamed generic-instantiation and autogenerated-equality symbols a
  changed layout's name hash produces — same body, new hash) — the allowlist names
  ...
  or jump, plain or method — is residue); the synctest legs and
...
- **Shared-helper extractions.** Where the bubble drain or the forced-cycle protocol must run the
  ...
  limit for code. The admission is pairwise and checkable: the extracted helper's body is the
  stock loop modulo its own prologue/epilogue, register assignment, and the recorded layout
...
- **Export-data inline-body availability.** The fork's runtime instantiates `abi.TypeFor`
  ...
  may inline those micro-bodies where stock emits calls (observed: `internal/sync`'s
  `HashTrieMap` instantiations in package `sync`, a few instructions smaller than stock, with
  the `abi.TypeOf`/`NoEscape` symbols dropped). An admitted symbol's diff must consist solely
  of such call→inlined-micro-body substitutions; behavior is identical by construction
  (`NoEscape` is the identity, `TypeOf` a type-descriptor load).
- **Fence-wrapper splits.** Where the hooked symbol's body is generated (`mksyscall`) or
  assembly, the body keeps its stock text under a split name and the stock-named symbol becomes
  the fenced wrapper: `syscall.Close`/`Fstat`/`Seek`/`Fsync`/`Fdatasync`/`Flock`/`Kill`/
  `Madvise`/`Mprotect` over their generated `closeFD`/`fstatFD`/`seekFD`/lowercase twins (the
  `//sys` lines carry the renames, so regeneration converges), and linux/amd64's
  `syscall.gettimeofday` over the assembly `rawGettimeofday`. Untagged the wrapper is one call
  (its hook sits behind the bare-constant guard); the pair equals the stock symbol modulo that
  call, and a caller that inlines the wrapper differs modulo the call-target name alone.
...
(stale-compiler builds silently pass). The `VERSION` file carries a `-dst.N` suffix
(`goX.Y.Z-dst.N`, see releases.md), so this checkout's tool IDs differ from stock's and from any sibling worktree
...
clean-cache rule stands unchanged. All legs gate green; a red leg is a regression against
this section.
