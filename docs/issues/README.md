...
| Issue | Lands | Summary |
|-------|-------|---------|
| [dst-public-package-location](dst-public-package-location.md) | before DST public API is released (pre-v1 clean break) | Relocate the public `runtime/dst.Run` wrapper out of `src/runtime/dst` to mirror `testing/synctest` (`testing/simulation`/`testing/dst`); the runtime-internal mechanism stays in `runtime`. |
