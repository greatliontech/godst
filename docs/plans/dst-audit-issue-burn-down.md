...
- [x] 45. dialer and listener ephemeral counters are host-scoped (`dst-net-port-allocator-cross-host-coupling`)
- [x] 46. explicit LocalAddr validates source ownership, reserves before parking, and releases on every failure (`dst-net-local-bind-lifecycle-incomplete`)
- [x] 47. IP-less wildcard LocalAddr behavior is modeled or recorded (`dst-net-wildcard-localaddr-bind-collapse`)
...
- [x] 49. connection ownership publishes atomically with listener handoff and teardown visibility (`dst-net-accept-handoff-precedes-ownership`)
- [x] 50. stateful connection/listener operations reject stale epochs; address accessors stay immutable and Close remains local cleanup (`dst-net-handles-cross-run-epochs`)
...
- [x] 55. PID exhaustion rolls back node identity and victim registration completely (`dst-process-pid-exhaustion-leaks-stamp`)
...
- [x] 57. Host NumCPU is exact over its accepted public range or rejected before publication (`dst-host-numcpu-wraps-int32`)
- [x] 58. CrashHost tracks root-process host ancestry through nested Host bodies (`dst-host-crash-misses-nested-root-goroutine`)
- [x] 59. caller-gate readers cannot be killed while parked holding the read side (`dst-sim-guarded-reader-killed-while-parked`)
- [x] 60. guard helpers structurally own complete per-API mutation callbacks, with killing tests for inactive, active, and panic boundaries (`dst-sim-guard-hold-extent-unpinned`)
