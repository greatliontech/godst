...
// hook-inert (DST-L2-4). See dst_on.go for the active build.
const dstHookEnabled = false

// dstSyncAcquire is a no-op outside a -tags dst -race build: the DST Level-2
// sync-decision auto-hook compiles to nothing, so Mutex methods are hook-inert in a
// non-dst build, a plain -race build, and a -tags dst build without -race
// (DST-L2-4). See dst_on.go for the active form.
